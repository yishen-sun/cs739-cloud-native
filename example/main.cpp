...
std::string download_and_encode_file(Aws::S3::S3Client const& client, Aws::String const& bucket,
                                     Aws::String const& key, Aws::String& encoded_output);
...
std::string encode(Aws::IOStream& stream, Aws::String& output) {
    ...
    while (stream.good()) {
        ...
        if (bytesRead > 0) {
            bits.insert(bits.end(), (unsigned char*)streamBuffer,
